...
/**
 * @brief A lightweight view object providing matrix-like operations. Intended to be used for submatrices.
 * 
 * @tparam T 
 */
template <class T>
MatrixView<T>::MatrixView(T* mem, int n, int m, int row_size): mem(mem), _dim({n, m}), mem_row_size(row_size) {}

template <class T>
const Dim_t& MatrixView<T>::dim() const {return _dim;}

template <class T>
T& MatrixView<T>::item(int i, int j) {
    return mem[i * mem_row_size + j];
}
template <class T>
const T& MatrixView<T>::citem(int i, int j) const {
    return mem[i * mem_row_size + j];
}
template <class T>
...
MatrixView<T> MatrixView<T>::operator*(const MatrixView<T>& b) const {
    ...
    Matrix<T>::Matrix(int m, int n)
        : MatrixView<T>::mem(new T[m * n]),
          MatrixView<T>::_dim({m, n}),
          MatrixView<T>::mem_row_size(n) {}  // Note: any matrix elements accessed immediately after this is undefined
    template <class T>
    ...
    template <class T>
    Matrix<T>::~Matrix() { delete[] this->mem; }
    template <class T>
    MatrixView<T>& Matrix<T>::sub(int i, int j, int n, int m) {
        views.push_back(MatrixView<T>(this->mem+i*this->mem_row_size+j, n, m, this->mem_row_size));
        return views.back();
    }
    template <class T>
